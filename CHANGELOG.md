...
### Added
- **Include Row Number Toggle** (`includeRowNumber`): New boolean parameter for `readRows` and `filterRows` operations
  - When `true` (default), `_rowNumber` is included in the output — maintains full backward compatibility
  - When `false`, `_rowNumber` is omitted from the output, keeping data clean for read-only workflows
  - Only shown in the UI for `readRows` and `filterRows`; write/delete operations are unaffected

### Tests
- Added unit tests for `includeRowNumber` toggle covering both `readRows` and `filterRows`
- All tests passing
